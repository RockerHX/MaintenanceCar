...
typedef NS_ENUM(NSUInteger, SCOderDetailAlertType) {
    ...
    SCOderAlertDetailTypeCancelReserve
};

typedef NS_ENUM(NSUInteger, SCOderDetailMenuType) {
...
@implementation SCOderDetailViewController
    ...
    if (_detail)
    {
        switch (indexPath.section)
        {
            case 1:
            {
                if (indexPath.row)
                {
                    cell = [tableView dequeueReusableCellWithIdentifier:@"SCOderDetailProgressCell" forIndexPath:indexPath];
                    [(SCOderDetailProgressCell *)cell displayCellWithDetail:_detail index:(indexPath.row-1)];
                }
                else
                    cell = [tableView dequeueReusableCellWithIdentifier:@"SCOderDetailPromptCell" forIndexPath:indexPath];
            }
                break;
    ...
    if (_detail)
    {
        switch (indexPath.section)
        {
            case 1:
            {
                if (indexPath.row)
                {
                    if(!_oderDetailProgressCell)
                        _oderDetailProgressCell = [tableView dequeueReusableCellWithIdentifier:@"SCOderDetailProgressCell"];
                    height = [_oderDetailProgressCell displayCellWithDetail:_detail index:(indexPath.row-1)];
                }
                else
                    height = 43.0f;
            }
    ...
    [[SCAPIRequest manager] startOderDetailAPIRequestWithParameters:parameters success:^(AFHTTPRequestOperation *operation, id responseObject) {
        if (operation.response.statusCode == SCAPIRequestStatusCodeGETSuccess)
        {
            ...
            switch (statusCode)
            {
                ...
                {
                    _detail = [[SCOderDetail alloc] initWithDictionary:responseObject[@"data"] error:nil];
...
- (void)showCancelAlert
{
    ...
    [moreMenu show:^(NSInteger selectedIndex) {
        ...
        {
            case SCOderDetailMenuTypeCancelReservetion:
                [weakSelf showAlertWithTitle:@"您确定要取消此订单吗？" message:nil delegate:self tag:SCOderAlertDetailTypeCancelReserve cancelButtonTitle:@"否" otherButtonTitle:@"是"];
                break;
